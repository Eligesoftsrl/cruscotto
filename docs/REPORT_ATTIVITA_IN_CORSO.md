# Cruscotto HR — Refactoring e Migrazione · Attività in corso

**Preparato da:** Perfexia Srl
**Progetto:** Cruscotto HR — Sistema di monitoraggio HR della Pubblica Amministrazione
**Oggetto:** Panoramica delle attività attualmente in corso sul codice, dallo
stato iniziale (repository generata con Lovable) allo stato attuale di lavorazione, con
esempi di codice *prima/dopo* e schermate dell'applicazione.
...
di prototipazione rapida) ed è **in fase di ristrutturazione** per renderlo pulito,
manutenibile, sicuro e scalabile, **preservandone le funzionalità** per l'utente
finale. Le attività attualmente in corso, in estrema sintesi:

- Stiamo introducendo un'**architettura a livelli** (Service Layer) che separa nettamente
  l'accesso ai dati (Supabase) dalla logica di presentazione (i componenti a schermo).
- Stiamo **sostituendo i dati finti (mock)** delle sezioni operative con i **dati reali**
  del data warehouse; i pochi indicatori privi di tabella sorgente vengono chiaramente
  segnalati con un badge "dato dimostrativo".
- Stiamo rendendo la **configurazione Supabase "environment-aware"**: lo stesso identico
  codice usa il database cloud in produzione e quello locale in sviluppo, senza modifiche
  manuali.
- Stiamo **rimuovendo le credenziali/URL scritte a mano nel codice** (hardcoded),
  sostituendole con la variabile d'ambiente `VITE_SUPABASE_URL`.
- Stiamo **correggendo** i bug e i warning di console segnalati.
...
coprogettazione con il committente.
...
| Aspetto | Prima (Lovable) | Stato attuale (in lavorazione) |
|--------|------------------|--------------------|
...
> Dopo aver connesso l'app ai dati reali, questa fase punta a rendere il codice **solido,
> professionale e manutenibile** — il percorso in corso da "prototipo Lovable che
> funziona" verso un "prodotto software di livello enterprise". Anche qui: **esempi di
> codice reali Prima/Dopo**, perché la differenza si vede nel dettaglio.
...
**Ad oggi: `0 errori` di lint** (restano solo avvisi non bloccanti, legati alla
...
**Ad oggi:** **nessun componente** importa più direttamente `mockData`; l'unica porta
...
- **Console del browser** monitorata (verifica anche via test automatico).
- **Unit test** delle trasformazioni core in esecuzione.
...
*Documento predisposto a corredo delle attività di refactoring in corso. Per il dettaglio
puntuale delle lavorazioni fare riferimento allo storico dei commit del repository.*
