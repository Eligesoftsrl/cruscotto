...
import {
  DfpOverviewFilters,
  DFP_FILTER_DEFAULTS,
  type DfpOverviewFilterValues,
} from "./DfpOverviewFilters";
  ...
  return [
    {
      ...
      value: pt.toLocaleString("it-IT"),
      delta: `${kpiOverview.personaleTotaleVar > 0 ? "+" : ""}${kpiOverview.personaleTotaleVar}%`,
      deltaType: kpiOverview.personaleTotaleVar > 0 ? "up" : "down",
      icon: Users,
    ...
    },
    {
      ...
      value: `${eta}`,
      suffix: "anni",
      delta: `${kpiOverview.etaMediaVar > 0 ? "+" : ""}${kpiOverview.etaMediaVar}`,
    ...
    {
      label: "Tasso Turnover",
      value: `${turnover}%`,
      delta: `${kpiOverview.turnoverVar > 0 ? "+" : ""}${kpiOverview.turnoverVar}%`,
      deltaType: kpiOverview.turnoverVar > 0 ? "up" : "down",
      icon: TrendingUp,
    ...
    },
    {
      label: "% Formati",
      value: `${formazione}%`,
      delta: `${kpiOverview.formazioneVar > 0 ? "+" : ""}${kpiOverview.formazioneVar}%`,
    ...
    },
    {
      label: "Lavoro Agile",
      value: `${agile}%`,
      delta: `${kpiOverview.lavoroAgileVar > 0 ? "+" : ""}${kpiOverview.lavoroAgileVar}%`,
      deltaType: "up" as const,
      icon: Briefcase,
...
export const OverviewHome = () => {
  ...
  const filteredBenchmark = useMemo(() => {
    ...
    return {
      etaMedia: {
        amministrazione: +(benchmarkData.etaMedia.amministrazione + offset).toFixed(1),
        cluster: benchmarkData.etaMedia.cluster,
      },
      donnePerc: {
        amministrazione: +(benchmarkData.donnePerc.amministrazione + offset * 2).toFixed(1),
        cluster: benchmarkData.donnePerc.cluster,
      },
      turnover: {
        amministrazione: +(benchmarkData.turnover.amministrazione + offset).toFixed(1),
        cluster: benchmarkData.turnover.cluster,
      },
      formazionePerc: {
        amministrazione: +(benchmarkData.formazionePerc.amministrazione + offset * 1.5).toFixed(1),
        cluster: benchmarkData.formazionePerc.cluster,
      },
      lavoroAgilePerc: {
        amministrazione: +(benchmarkData.lavoroAgilePerc.amministrazione + offset * 2).toFixed(1),
        cluster: benchmarkData.lavoroAgilePerc.cluster,
      },
    };
  }, [factor]);
  ...
  const filterLabel =
    isDfp && activeFilterParts.length > 0 ? ` · Filtro: ${activeFilterParts.join(", ")}` : "";
          ...
          {[
            ...
            return (
              <div key={item.key} className="text-center">
                ...
                >
                  Gap: {gap > 0 ? "+" : ""}
                  {gap.toFixed(1)} {item.suffix.replace("%", "pp")}
                </div>
              </div>
            );
          })}
