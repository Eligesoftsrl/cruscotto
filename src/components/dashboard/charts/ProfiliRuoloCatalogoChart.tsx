...
export const ProfiliRuoloCatalogoChart = () => {
  ...
  const countBy = (key: keyof ProfileRow) => {
    ...
    for (const r of rows) { const k = r[key] as string; map[k] = (map[k] || 0) + 1; }
    return Object.entries(map).sort(([, a], [, b]) => b - a).map(([name, value]) => ({ name: name.length > 25 ? name.substring(0, 23) + "…" : name, value }));
  };
  ...
  const columns = [
    { key: "famiglia", header: "Famiglia professionale", render: (r: ProfileRow) => <span className="font-medium text-foreground text-[11px] max-w-[140px] block">{r.famiglia}</span> },
    { key: "ambito", header: "Ambito", render: (r: ProfileRow) => <span className="text-muted-foreground text-[11px] max-w-[140px] block">{r.ambito}</span> },
    { key: "areaContrattuale", header: "Area contrattuale", render: (r: ProfileRow) => <span className="text-muted-foreground text-[11px]">{r.areaContrattuale}</span> },
    { key: "profiloRuolo", header: "Profilo di ruolo", render: (r: ProfileRow) => <span className="text-foreground text-[11px] max-w-[180px] block">{r.profiloRuolo}</span> },
    { key: "origine", header: "Origine", align: "center" as const, render: (r: ProfileRow) => <span className="text-muted-foreground text-[11px]">{r.origine}</span> },
    { key: "fteProgrammati", header: "Risorse progr.", align: "right" as const, render: (r: ProfileRow) => <span className="text-muted-foreground">{r.fteProgrammati}</span> },
    { key: "fteAssegnati", header: "Risorse in servizio", align: "right" as const, render: (r: ProfileRow) => <span className="text-muted-foreground">{r.fteAssegnati}</span> },
  ];
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-card border rounded-xl p-5 space-y-3">
        <h3 className="text-[15px] font-bold text-foreground">Distribuzione dei Profili di Ruolo per Origine e Famiglia professionale</h3>
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <div>
            <p className="text-xs font-semibold text-muted-foreground text-center mb-2">Per origine (Minerva, SIPrO)</p>
            <ResponsiveContainer width="100%" height={240}>
              <PieChart>
                <Pie data={originData} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={90} label={({ name, value }) => `${name}: ${value}`} labelLine>
          ...
          <div>
            <p className="text-xs font-semibold text-muted-foreground text-center mb-2">Per famiglia professionale, ambito, area e profilo</p>
            <ResponsiveContainer width="100%" height={240}>
              <BarChart data={distData} margin={{ top: 5, right: 20, left: 0, bottom: 5 }}>
                ...
                <Tooltip contentStyle={{ background: "hsl(var(--card))", border: "1px solid hsl(var(--border))", borderRadius: 8, fontSize: 12 }} />
                <Bar dataKey="value" name="Conteggio" fill={TEAL} radius={[4, 4, 0, 0]} maxBarSize={50}>
      ...
      <div className="bg-card border rounded-xl p-5 space-y-3">
        <h3 className="text-[15px] font-bold text-foreground">Dettaglio per Famiglia professionale e Ambito di Ruolo</h3>
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <div>
            <p className="text-xs font-semibold text-muted-foreground text-center mb-2">Per famiglia professionale</p>
            <ResponsiveContainer width="100%" height={Math.max(200, famigliaData.length * 32)}>
              <BarChart data={famigliaData} layout="vertical" margin={{ top: 5, right: 30, left: 10, bottom: 5 }}>
                <CartesianGrid strokeDasharray="3 3" horizontal={false} stroke="hsl(var(--border))" />
                <XAxis type="number" tick={{ fontSize: 10 }} stroke="hsl(var(--muted-foreground))" />
                <YAxis dataKey="name" type="category" width={150} tick={{ fontSize: 9 }} stroke="hsl(var(--muted-foreground))" />
                <Tooltip contentStyle={{ background: "hsl(var(--card))", border: "1px solid hsl(var(--border))", borderRadius: 8, fontSize: 12 }} />
                <Bar dataKey="value" name="Profili" fill={TEAL} radius={[0, 4, 4, 0]} maxBarSize={24}>
          ...
          <div>
            ...
            <ResponsiveContainer width="100%" height={Math.max(200, ambitoData.length * 32)}>
              <BarChart data={ambitoData} layout="vertical" margin={{ top: 5, right: 30, left: 10, bottom: 5 }}>
                <CartesianGrid strokeDasharray="3 3" horizontal={false} stroke="hsl(var(--border))" />
                <XAxis type="number" tick={{ fontSize: 10 }} stroke="hsl(var(--muted-foreground))" />
                <YAxis dataKey="name" type="category" width={150} tick={{ fontSize: 9 }} stroke="hsl(var(--muted-foreground))" />
                <Tooltip contentStyle={{ background: "hsl(var(--card))", border: "1px solid hsl(var(--border))", borderRadius: 8, fontSize: 12 }} />
                <Bar dataKey="value" name="Profili" fill={BLUE} radius={[0, 4, 4, 0]} maxBarSize={24}>
                  <LabelList dataKey="value" position="insideRight" style={{ fontSize: 11, fontWeight: 700, fill: "#fff" }} />
                </Bar>
              </BarChart>
            </ResponsiveContainer>
      ...
      {/* Resources table */}
      <div className="bg-card border rounded-xl p-5 space-y-3">
        <h3 className="text-[15px] font-bold text-foreground">Risorse programmate e assegnate per i profili di ruolo</h3>
        <p className="text-xs text-muted-foreground">Dettaglio delle risorse programmate (PIAO/PEG) e delle risorse effettivamente in servizio per ciascun profilo di ruolo.</p>
