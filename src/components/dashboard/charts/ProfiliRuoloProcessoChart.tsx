    ...
    const load = async () => {
      ...
      const [procRes, fasiRes, profFasiRes, profRuoloRes, ambitoRes, areaRes] = await Promise.all([
        ...
        sipoFrom("ft_sipo_profili_di_ruolo_fasi").select(
          "profilo_fase_id, fase_id, sipo_profilo_di_ruolo_id, fte_programmati, fte_assegnati",
        ),
        sipoFrom("lk_sipo_profili_di_ruolo")
          .select("profilo_ruolo_id, profilo_ruolo, id_ambito_ruolo, id_area_contrattuale, ente_id")
      ...
      const profRuoloMap = new Map(
        (profRuoloRes.data ?? []).map((p: any) => [p.profilo_ruolo_id, p]),
      );
      ...
      for (const pfs of profFasiByFase.values()) {
        for (const pf of pfs)
          if (pf.sipo_profilo_di_ruolo_id) relevantIds.add(pf.sipo_profilo_di_ruolo_id);
      }
      ...
      for (const id of relevantIds) {
        ...
        const name =
          pr.profilo_ruolo?.length > 20
            ? pr.profilo_ruolo.substring(0, 18) + "…"
            : pr.profilo_ruolo;
        ambCount[amb.length > 20 ? amb.substring(0, 18) + "…" : amb] =
          (ambCount[amb.length > 20 ? amb.substring(0, 18) + "…" : amb] || 0) + 1;
        arCount[ar] = (arCount[ar] || 0) + 1;
        prCount[name] = (prCount[name] || 0) + 1;
      }

      setAmbitoData(
        Object.entries(ambCount)
          .sort(([, a], [, b]) => b - a)
          .map(([name, value]) => ({ name, value })),
      );
      setAreaData(
        Object.entries(arCount)
          .sort(([, a], [, b]) => b - a)
          .map(([name, value]) => ({ name, value })),
      );
      setProfData(
        Object.entries(prCount)
          .sort(([, a], [, b]) => b - a)
          .slice(0, 10)
          .map(([name, value]) => ({ name, value })),
      );
    ...
    <div className="space-y-4">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div>
            <p className="text-xs font-semibold text-muted-foreground text-center mb-2">
              Numero profili di ruolo per Processo/Fase
            </p>
          ...
          <div>
            <p className="text-xs font-semibold text-muted-foreground text-center mb-2">
              Confronto risorse programmate (PIAO/PEG) e risorse in servizio
            </p>
            <ResponsiveContainer width="100%" height={300}>
              <BarChart data={chartProc} margin={{ top: 5, right: 10, left: -5, bottom: 50 }}>
                <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="hsl(var(--border))" />
      ...
      <div className="bg-card border rounded-xl p-5 space-y-3">
        <h3 className="text-[15px] font-bold text-foreground">
          Distribuzione per Ambito, Area contrattuale e Profilo
        </h3>
