  ...
  useEffect(() => {
    const load = async () => {
      ...
      for (const f of (fasiRes.data ?? []) as any[]) {
        if (procIds.has(f.processo_id))
          fasiCount.set(f.processo_id, (fasiCount.get(f.processo_id) || 0) + 1);
      }
      ...
      const tableRows: ProcCritRow[] = (procRes.data as any[]).map((p) => {
        ...
        return {
          ...
          nrFasi: fasiCount.get(p.processo_id) ?? 0,
          nrCriticita: nrCrit,
          macroCriticita: macroDesc,
          semplificato: sempl,
        };
      });
  ...
  const columns = [
    {
      key: "denominazione",
      header: "Processo",
      render: (r: ProcCritRow) => (
        <span className="font-medium text-foreground max-w-[180px] block">{r.denominazione}</span>
      ),
    },
    {
      key: "nrFasi",
      ...
      render: (r: ProcCritRow) => <span className="text-muted-foreground">{r.nrFasi}</span>,
    },
    {
      key: "nrCriticita",
      header: "Nr criticità",
      align: "right" as const,
      render: (r: ProcCritRow) => <span className="text-muted-foreground">{r.nrCriticita}</span>,
    },
    {
      key: "macroCriticita",
      header: "Macro-criticità",
      render: (r: ProcCritRow) => (
        <span className="text-muted-foreground text-[11px] max-w-[200px] block">
          {r.macroCriticita}
        </span>
      ),
    },
    {
      ...
      render: (r: ProcCritRow) => <span className="text-muted-foreground">{r.semplificato}</span>,
    },
  ];
    ...
    <div className="space-y-4">
      ...
      <div className="bg-card border rounded-xl p-5 space-y-3">
        <h3 className="text-[15px] font-bold text-foreground">
          Semplificazioni e Criticità segnalate
        </h3>
        <p className="text-xs text-muted-foreground">
          Criticità segnalate sulle fasi dei processi ed eventuali vincoli alle semplificazioni.
        </p>
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div>
            <p className="text-xs font-semibold text-muted-foreground text-center mb-2">
              Frequenza Criticità per Macro Criticità
            </p>
