...
export const DigitalizzazioneFasiChart = () => {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      const [procRes, fasiRes, digLkRes, outsLkRes, agileLkRes] = await Promise.all([
        procQ,
        sipoFrom("ft_sipo_fasi").select(
          "fase_id, processo_id, livello_digitalizzazione_id, outsourcing_id, lavoro_agile_id, in_outsourcing, lavoro_agile",
        ),
      ...
      const digLkMap = new Map(
        (digLkRes.data ?? []).map((d: any) => [
          d.livello_digitalizzazione_id,
          d.descrizione ?? `Livello ${d.livello_digitalizzazione_id}`,
        ]),
      );
      const outsLkMap = new Map(
        (outsLkRes.data ?? []).map((o: any) => [o.outsourcing_id, o.descrizione]),
      );
      const agileLkMap = new Map(
        (agileLkRes.data ?? []).map((a: any) => [a.lavoro_agile_id, a.descrizione]),
      );
      ...
      const perProc: Record<
        number,
        { total: number; outs: number; agile: number; digLevels: number[] }
      > = {};
      ...
      for (const f of fasi) {
        ...
        const agileLabel =
          agileLkMap.get(f.lavoro_agile_id) ??
          (f.lavoro_agile === 1 ? "Sì" : "No, non sono previste attività esternalizzate");
        agileCounts[agileLabel] = (agileCounts[agileLabel] || 0) + 1;

        if (!perProc[f.processo_id])
          perProc[f.processo_id] = { total: 0, outs: 0, agile: 0, digLevels: [] };
        ...
        if (f.livello_digitalizzazione_id)
          perProc[f.processo_id].digLevels.push(f.livello_digitalizzazione_id);
      }
  ...
  const tableColumns = [
    ...
    },
    {
      ...
      render: (r: ProcFasiRow) => <span className="text-muted-foreground">{r.numFasi}</span>,
    },
    {
      key: "fasiEsternalizz",
      header: "Fasi con esternalizzazioni",
      align: "right" as const,
      render: (r: ProcFasiRow) => (
        <span className="text-muted-foreground">{r.fasiEsternalizz}</span>
      ),
    },
    {
      key: "fasiLavoroAgile",
      header: "Fasi in lavoro agile",
      align: "right" as const,
      render: (r: ProcFasiRow) => (
        <span className="text-muted-foreground">{r.fasiLavoroAgile}</span>
      ),
    },
    {
      key: "livelloDigPrevalente",
      header: "Livello digitalizzazione prevalente",
      render: (r: ProcFasiRow) => (
        <span className="text-muted-foreground">{r.livelloDigPrevalente}</span>
      ),
    },
  ];
    ...
    <div className="space-y-4">
      ...
      <div className="bg-card border rounded-xl p-5 space-y-4">
        <h3 className="text-[15px] font-bold text-foreground">
          Digitalizzazione e lavoro agile (Fasi)
        </h3>
        <p className="text-xs text-muted-foreground">
          Sintesi dei livelli di digitalizzazione e delle attività svolgibili in lavoro agile e
          sulle fasi dei processi.
        </p>
