...
export const UoDistributionChart = () => {
  ...
  const [benchmarkData, setBenchmarkData] = useState<{
    ...
    entiNames: string[];
    totals: { ente: string; total: number }[];
  }>({ levels: [], entiNames: [], totals: [] });
  ...
  useEffect(() => {
    const load = async () => {
      ...
      let query = sipoFrom("ft_sipo_uo")
        .select(
          "livello_gerarchico, livello_resp_id, ente_id, lk_sipo_livelli_resp_uo(descrizione)",
        )
      ...
      if (uoRes.data) {
        ...
        setByLevel(
          Object.entries(levelMap)
            ...
            .map(([k, v]) => ({ name: `${k}° livello`, value: v })),
        );
        setByResp(
        ...
        if (ids.length > 1) {
          ...
          for (const row of data) {
            ...
            perEnte[row.ente_id][row.livello_gerarchico] =
              (perEnte[row.ente_id][row.livello_gerarchico] || 0) + 1;
          ...
          const entiNames = ids.map((id) =>
            (entiMap.get(id) ?? `Ente ${id}`).replace("Comune di ", ""),
          );
      ...
      {loading ? (
        ...
        <div className="bg-card border rounded-xl p-5 space-y-4">
          ...
          <p className="text-xs text-muted-foreground">
            Numero di Unità Organizzative per livello gerarchico, confrontate tra gli enti
            selezionati.
          </p>
                ...
                >
                  ...
                  {benchmarkData.entiNames.map((name, i) => (
                    <Bar
                      key={name}
                      dataKey={name}
                      fill={ENTE_COLORS[i % ENTE_COLORS.length]}
                      radius={[3, 3, 0, 0]}
                      maxBarSize={32}
                    />
                  ))}
        ...
        <div className="bg-card border rounded-xl p-5 space-y-2">
          <h3 className="text-[15px] font-bold text-foreground">
            Distribuzione delle Unità Organizzative per Livello Gerarchico e Livello di
            Responsabilità
          </h3>
            ...
            <div>
              ...
              <ResponsiveContainer width="100%" height={280}>
                <BarChart data={byLevel} margin={{ top: 10, right: 10, left: -10, bottom: 0 }}>
                  ...
                  <Bar
                    dataKey="value"
                    name="Numero Unità Organizzative"
                    radius={[4, 4, 0, 0]}
                    maxBarSize={48}
                  >
                    {byLevel.map((_, i) => (
                ...
                <PieChart>
                  ...
                  <Tooltip
                    ...
                    formatter={(value: number) => [
                      `${value} (${((value / totalResp) * 100).toFixed(0)}%)`,
                      "",
                    ]}
                  />
                  <Legend
