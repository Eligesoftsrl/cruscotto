  ...
  useEffect(() => {
    const load = async () => {
      ...
      let q = sipoFrom("ft_sipo_processi")
        .select(
          "denominazione, giorni_previsti, tempo_medio_effettivo, picchi_stagionali, presidio_continuativo, picchi_frequenza_id, picchi_intensita_id",
        )
        .is("data_fine", null);
      ...
      const freqMap = new Map(
        (freqRes.data ?? []).map((f: any) => [f.picchi_frequenza_id, f.frequenza]),
      );
      const intMap = new Map(
        (intRes.data ?? []).map((i: any) => [i.picchi_intensita_id, i.intensita]),
      );

      setTempi(
        procs
          .filter((p) => p.giorni_previsti || p.tempo_medio_effettivo)
          .map((p) => ({
            name:
              p.denominazione.length > 20
                ? p.denominazione.substring(0, 18) + "…"
                : p.denominazione,
  ...
  const tempiColumns = [
    {
      ...
      render: (r: TempoRow) => <span className="font-medium text-foreground">{r.name}</span>,
    },
    {
      key: "previsto",
      header: "Tempo previsto (giorni)",
      align: "right" as const,
      render: (r: TempoRow) => <span className="text-muted-foreground">{r.previsto}</span>,
    },
    {
      key: "effettivo",
      header: "Tempo effettivo (giorni)",
      align: "right" as const,
      render: (r: TempoRow) => <span className="text-muted-foreground">{r.effettivo}</span>,
    },
  ];
    ...
    <div className="space-y-6">
      ...
      <div className="bg-card border rounded-xl p-5 space-y-3">
        ...
        <p className="text-xs text-muted-foreground">
          Tempi di esecuzione previsti/effettivi dei processi censiti.
        </p>
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <div>
            <p className="text-xs font-semibold text-muted-foreground text-center mb-2">
              Confronto tra tempi di esecuzione previsti e tempi effettivi medi
            </p>
            <ResponsiveContainer width="100%" height={300}>
              <BarChart
                data={tempi.slice(0, 15)}
      ...
      <div className="bg-card border rounded-xl p-5 space-y-3">
        ...
        <p className="text-xs text-muted-foreground">
          Informazioni sui picchi stagionali e sulla necessità di presidio continuativo.
        </p>
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div>
            <p className="text-xs font-semibold text-muted-foreground text-center mb-2">
              % Processi che richiedono presidio continuativo
            </p>
            <ResponsiveContainer width="100%" height={220}>
              <PieChart>
                <Pie
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 pt-2">
          <div>
            ...
            <p className="text-xs text-muted-foreground">
              Senza picchi stagionali:{" "}
              <strong className="text-foreground">{picchiCounts.senza}</strong>
            </p>
            <p className="text-xs text-muted-foreground">
              Con picchi stagionali: <strong className="text-foreground">{picchiCounts.con}</strong>
            </p>
          </div>
          <div>
            ...
            <p className="text-xs text-muted-foreground">
              Processi senza presidio continuativo:{" "}
              <strong className="text-foreground">{presidioCounts.senza}</strong>
            </p>
            <p className="text-xs text-muted-foreground">
              Processi con presidio continuativo:{" "}
              <strong className="text-foreground">{presidioCounts.con}</strong>
            </p>
          </div>
        </div>
      </div>
