...
export const ProcessiDettaglioTable = () => {
  ...
  const [rilevanzaData, setRilevanzaData] = useState<
    { name: string; obiettivi: string; rilevanza: string }[]
  >([]);
  ...
  useEffect(() => {
    const load = async () => {
      ...
      let procQuery = sipoFrom("ft_sipo_processi")
        .select(
          "processo_id, denominazione, tipologia_id, obiettivo_strategico_id, grado_rilevanza_id, coinvolgimento_amministrazioni, presidio_continuativo, picchi_stagionali, processo_semplificazione_id, ente_id",
        )
        .is("data_fine", null);
      ...
      const semplMap = new Map(
        (semplRes.data ?? []).map((s: any) => [s.semplificazione_id, s.descrizione]),
      );
      ...
      const tableRows: ProcessRow[] = (procRes.data as any[]).map((p) => {
        ...
        const semplificato =
          semplDesc && semplDesc !== "No"
            ? semplDesc.includes("parte")
              ? "Sì, in parte"
              : "No"
            : "No";
  ...
  const mainColumns = [
    ...
    },
    {
      key: "funzione",
      header: "Funzione",
      render: (r: ProcessRow) => <span className="text-muted-foreground">{r.funzione}</span>,
    },
    {
      key: "tipologia",
      header: "Tipologia",
      render: (r: ProcessRow) => (
        <span className="text-muted-foreground max-w-[180px] block">{r.tipologia}</span>
      ),
    },
    {
      key: "obiettivi",
      header: "Obiettivi Strategici",
      render: (r: ProcessRow) => <span className="text-muted-foreground">{r.obiettivi}</span>,
    },
    {
      key: "rilevanza",
      header: "Rilevanza",
      render: (r: ProcessRow) => <span className="text-muted-foreground">{r.rilevanza}</span>,
    },
    {
      key: "altre_amm",
      header: "Altre amm.",
      render: (r: ProcessRow) => <span className="text-muted-foreground">{r.altre_amm}</span>,
    },
    {
      key: "semplificato",
      header: "Semplificato",
      render: (r: ProcessRow) => <span className="text-muted-foreground">{r.semplificato}</span>,
    },
    {
      key: "presidio",
      header: "Presidio",
      render: (r: ProcessRow) => <span className="text-muted-foreground">{r.presidio}</span>,
    },
    {
      key: "picchi",
      header: "Picchi",
      render: (r: ProcessRow) => <span className="text-muted-foreground">{r.picchi}</span>,
    },
    {
      key: "criticita",
      header: "Criticità",
      render: (r: ProcessRow) => <span className="text-muted-foreground">{r.criticita}</span>,
    },
  ];
  ...
  const rilColumns = [
    {
      key: "name",
      header: "Nome Processo",
      render: (r: any) => <span className="font-medium text-foreground">{r.name}</span>,
    },
    {
      key: "obiettivi",
      header: "Obiettivi strategici",
      render: (r: any) => <span className="text-muted-foreground">{r.obiettivi}</span>,
    },
    {
      key: "rilevanza",
      header: "Grado di rilevanza",
      render: (r: any) => <span className="text-muted-foreground">{r.rilevanza}</span>,
    },
  ];
    ...
    <div className="space-y-6">
      ...
      <div className="bg-card border rounded-xl p-5 space-y-3">
        ...
        <p className="text-xs text-muted-foreground">
          Dettaglio completo dei processi mappati con attributi chiave.
        </p>
      ...
      <div className="bg-card border rounded-xl p-5 space-y-3">
        ...
        <p className="text-xs text-muted-foreground">
          Collegamento dei processi agli obiettivi strategici.
        </p>
