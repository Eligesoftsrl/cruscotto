...
interface MacroFreq { name: string; value: number }
...
export const CriticitaUoChart = () => {
      ...
      if (critRes.data && uoRes.data && lkRes.data) {
        ...
        const allRows = critRes.data
          .map((r: any) => {
            ...
            return { uo: uo.denominazione, criticita: lk.descrizione, macroCategoria: lk.categoria, enteId: uo.ente_id };
          })
        ...
        setRows(filtered.map(({ uo, criticita, macroCategoria }) => ({ uo, criticita, macroCategoria })));

        const catMap: Record<string, number> = {};
        for (const row of filtered) catMap[row.macroCategoria] = (catMap[row.macroCategoria] || 0) + 1;
        setMacroFreq(Object.entries(catMap).sort(([, a], [, b]) => b - a).map(([k, v]) => ({ name: k, value: v })));
        ...
        if (ids.length > 1) {
          ...
          for (const r of filtered) {
            ...
            perEnte[(r as any).enteId][r.macroCategoria] = (perEnte[(r as any).enteId][r.macroCategoria] || 0) + 1;
          ...
          setBenchmarkRows(
            ids.map((id) => {
              ...
              cats.forEach((c) => { row[c] = perEnte[id]?.[c] ?? 0; });
              return row;
            })
          );
        }
  ...
  const CAT_COLORS = [
    "hsl(175, 60%, 50%)", "hsl(210, 64%, 45%)", "hsl(330, 55%, 55%)",
    "hsl(40, 90%, 55%)", "hsl(120, 45%, 45%)", "hsl(0, 60%, 55%)",
    "hsl(270, 50%, 55%)", "hsl(30, 80%, 50%)",
  ];
            ...
            <div>
              ...
              <ResponsiveContainer width="100%" height={320}>
                <BarChart data={benchmarkRows} margin={{ top: 10, right: 30, left: -5, bottom: 5 }}>
                  ...
                  <YAxis tick={{ fontSize: 10 }} stroke="hsl(var(--muted-foreground))" />
                  <Tooltip contentStyle={{ background: "hsl(var(--card))", border: "1px solid hsl(var(--border))", borderRadius: 8, fontSize: 12 }} />
                  <Bar dataKey="totale" name="Totale criticità" radius={[3, 3, 0, 0]} maxBarSize={48}>
              ...
              {totalPages > 1 && (
                <div className="flex items-center justify-between mt-2">
                  <div className="flex items-center gap-1">
                    ...
                    <button onClick={() => setPage(Math.max(0, page - 1))} disabled={page === 0} className="px-2 py-1 text-xs rounded border disabled:opacity-40">‹</button>
                    {Array.from({ length: Math.min(5, totalPages) }, (_, i) => {
                      const start = Math.max(0, Math.min(page - 2, totalPages - 5));
                      const p = start + i;
                      if (p >= totalPages) return null;
                      return (<button key={p} onClick={() => setPage(p)} className={`px-2 py-1 text-xs rounded border ${p === page ? "bg-primary text-primary-foreground" : ""}`}>{p + 1}</button>);
                    })}
                    <button onClick={() => setPage(Math.min(totalPages - 1, page + 1))} disabled={page >= totalPages - 1} className="px-2 py-1 text-xs rounded border disabled:opacity-40">›</button>
                  ...
                  <span className="text-[11px] text-muted-foreground">{page + 1} di {totalPages} ({rows.length} elementi)</span>
                </div>
              )}
            </div>
            <div>
              ...
              <ResponsiveContainer width="100%" height={Math.max(200, macroFreq.length * 52)}>
                <BarChart data={macroFreq} layout="vertical" margin={{ top: 5, right: 40, left: 10, bottom: 5 }}>
                  ...
                  <Bar dataKey="value" name="N° UO" radius={[0, 4, 4, 0]} maxBarSize={32}>
                    {macroFreq.map((_, i) => (<Cell key={i} fill={BAR_COLOR} />))}
