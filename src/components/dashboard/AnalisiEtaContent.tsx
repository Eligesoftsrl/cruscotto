...
import { serieStoricaEta, filterOptions } from "@/fixtures";
...
export const AnalisiEtaContent = () => {
  ...
  const { distribuzioneEta, isLoading, error } = useEtaData(2023);
  ...
  const etaPercTotale = distribuzioneEta.map(r => ({ fascia: r.fascia, valore: Number(((r.totale / totalPersonale) * 100).toFixed(1)) }));
  const etaPercUomini = distribuzioneEta.map(r => ({ fascia: r.fascia, valore: Number(((r.uomini / totalUomini) * 100).toFixed(1)) }));
  const etaPercDonne = distribuzioneEta.map(r => ({ fascia: r.fascia, valore: Number(((r.donne / totalDonne) * 100).toFixed(1)) }));

  const renderBarChart = (title: string, data: { fascia: string; valore: number }[], color: string, showLabel?: boolean) => (
    <div className="chart-container flex-1 min-w-[220px]">
      ...
      <ResponsiveContainer width="100%" height={220}>
        <BarChart data={data} margin={{ top: 15, right: 5, left: 5, bottom: 5 }}>
          ...
          <Bar dataKey="valore" fill={color} radius={[3, 3, 0, 0]}
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-lg border bg-card p-5 space-y-5">
        ...
        <p className="text-sm text-muted-foreground">Ultimo aggiornamento anno 2023 · Serie storica 2012 - 2023</p>
        ...
        <div className="flex flex-wrap items-center gap-4">
          <div className="flex items-center gap-2">
            ...
            <select className="rounded border bg-background px-3 py-1.5 text-xs outline-none focus:ring-1 focus:ring-ring min-w-[300px]">
              {filterOptions.macrocategorie.map((o: string) => <option key={o}>{o}</option>)}
            </select>
          </div>
          <div className="flex items-center gap-2">
            <label className="text-xs font-medium text-foreground whitespace-nowrap">Categoria:</label>
            <select className="rounded border bg-background px-3 py-1.5 text-xs outline-none focus:ring-1 focus:ring-ring min-w-[300px]">
              {filterOptions.categorie.map((o: string) => <option key={o}>{o}</option>)}
            </select>
          </div>
        </div>
        ...
        <div className="chart-container">
            ...
            <div className="flex items-center gap-4">
              {(["uomini", "donne", "totale"] as const).map((mode) => (
                <label key={mode} className="flex items-center gap-1.5 cursor-pointer">
                  <input type="checkbox" checked={serieMode === mode} onChange={() => setSerieMode(mode)} className="rounded border-border" />
          ...
          <DemoDataBadge note="Serie storica età (2012-2023): dato dimostrativo, non ancora disponibile nelle tabelle dw_*." />
          ...
          <ResponsiveContainer width="100%" height={320}>
            <AreaChart data={serieStoricaEta}>
              ...
              <Tooltip />
              {AGE_BANDS.map((band, i) => (
                <Area key={band} type="monotone" dataKey={band} stackId="1" fill={BAND_COLORS[i]} stroke={BAND_COLORS[i]} fillOpacity={0.85} />
              ))}
            </AreaChart>
          </ResponsiveContainer>
        </div>
