      ...
      <ResponsiveContainer width="100%" height={220}>
        <BarChart data={data} margin={{ top: 15, right: 5, left: 5, bottom: 5 }}>
          ...
          <Bar
            ...
            label={
              showLabel !== false
                ? { position: "top", fontSize: 10, formatter: (v: number) => `${v}%` }
                : undefined
            }
          />
        </BarChart>
    ...
    <div className="space-y-6">
      <div className="space-y-1">
        <p className="text-sm font-semibold text-foreground">
          Dati del Conto Annuale - Rilevazione censuaria sulle amministrazioni pubbliche, effettuata
        ...
        <div className="flex flex-wrap items-center gap-4">
          <div className="flex items-center gap-2">
            ...
            <select className="rounded border bg-background px-3 py-1.5 text-xs outline-none focus:ring-1 focus:ring-ring min-w-[300px]">
              {filterOptions.macrocategorie.map((o: string) => (
          ...
          <div className="flex items-center gap-2">
            ...
            <select className="rounded border bg-background px-3 py-1.5 text-xs outline-none focus:ring-1 focus:ring-ring min-w-[300px]">
              {filterOptions.categorie.map((o: string) => (
                <option key={o}>{o}</option>
              ))}
            </select>
          </div>
              ...
              {(["uomini", "donne", "totale"] as const).map((mode) => (
                <label key={mode} className="flex items-center gap-1.5 cursor-pointer">
                  ...
                  <span className="text-xs text-foreground capitalize">
                    {mode === "totale" ? "Totale" : mode === "uomini" ? "Uomini" : "Donne"}
                  </span>
                </label>
              ))}
            </div>
      ...
      <div className="rounded-lg border bg-card p-5">
        <div className="flex items-center gap-3 mb-4">
          ...
          <h3 className="text-base font-bold text-foreground">
            Indicatori per cluster di amministrazione pubblica
          </h3>
        </div>
        <BenchmarkSection />
      </div>
