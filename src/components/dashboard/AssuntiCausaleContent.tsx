...
import {
  useAssuntiKpi, useAssuntiCausali, useAssuntiEvoluzione,
} from "@/hooks/useSchedaAssunti";
import type { Genere, AssuntiFiltri } from "@/services/ca/assuntiService";
...
/* Palette per le causali (barre e donut). */
const CAUSALE_COLORS = [
  "hsl(220,60%,50%)",
  "hsl(25,85%,55%)",
  "hsl(175,55%,42%)",
  "hsl(280,45%,55%)",
...
export const AssuntiCausaleContent = () => {
  ...
  const kpi = useAssuntiKpi(filtri);
  const causali = useAssuntiCausali(filtri);
  const evo = useAssuntiEvoluzione(filtri);

  const k = kpi.data;
  const causaliRows = causali.data ?? [];
  const evoRows = (evo.data ?? []).slice().sort((a, b) => a.anno - b.anno);

  // Barre Uomini/Donne: i campi uomini/donne NON risentono del filtro genere.
  // Applichiamo comunque uno sbiadimento visivo del genere non selezionato.
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap gap-4">
        <Kpi accent="hsl(220,60%,50%)" titolo="Totale assunti"
          valore={k ? nf.format(k.assunti) : "\u2014"} />
        <Kpi accent="hsl(25,85%,55%)" titolo="Causale prevalente"
          valore={k?.causale_prevalente ?? "\u2014"} />
        <Kpi accent="hsl(280,45%,55%)" titolo="% donne assunzioni" unita="%"
          valore={n1(k?.donne_pct)} />
        <Kpi accent="hsl(145,50%,42%)" titolo="Personale totale"
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <div className="rounded-lg border bg-card p-5">
          <h4 className="mb-3 text-sm font-semibold text-foreground">Assunzioni per causale — Uomini vs Donne</h4>
          <ResponsiveContainer width="100%" height={360}>
            <BarChart layout="vertical" data={causaliRows} margin={{ left: 10, right: 20 }}>
              ...
              <Bar dataKey="donne" stackId="s" fill="hsl(25,85%,55%)" fillOpacity={opDonne} radius={[0, 3, 3, 0]} />
            </BarChart>
          </ResponsiveContainer>
        </div>
      ...
      {/* ------------------- TREND ASSUNZIONI ------------------- */}
      <div className="rounded-lg border bg-card p-5">
        <h4 className="mb-3 text-sm font-semibold text-foreground">Trend assunzioni</h4>
        <ResponsiveContainer width="100%" height={300}>
          <AreaChart data={evoRows} margin={{ left: 0, right: 10 }}>
            <defs>
              <linearGradient id="gradAssunti" x1="0" y1="0" x2="0" y2="1">
                <stop offset="5%" stopColor="hsl(220,60%,50%)" stopOpacity={0.5} />
                <stop offset="95%" stopColor="hsl(220,60%,50%)" stopOpacity={0.05} />
              </linearGradient>
            </defs>
            <CartesianGrid strokeDasharray="3 3" stroke="hsl(220,15%,90%)" />
            <XAxis dataKey="anno" tick={{ fontSize: 10 }} />
            <YAxis tick={{ fontSize: 10 }} tickFormatter={(v: number) => nf.format(v)} />
            <Tooltip formatter={(v: number) => [nf.format(Number(v)), "Assunti"]} />
            <Area type="monotone" dataKey="assunti" name="Assunti" stroke="hsl(220,60%,50%)" strokeWidth={2} fill="url(#gradAssunti)" />
          </AreaChart>
        </ResponsiveContainer>
      </div>
      ...
      {/* ------------------- TABELLA DETTAGLIO ------------------- */}
      <div className="rounded-lg border bg-card p-5">
        <h4 className="mb-3 text-sm font-semibold text-foreground">Dettaglio per causale</h4>
