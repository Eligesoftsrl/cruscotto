...
import {
  SiproFilters,
  type SiproFilterValues,
  effectiveEnteIds,
} from "@/components/dashboard/charts/SiproFilters";
...
const StatoOrgView = () => {
  ...
  const [filters, setFilters] = useState<SiproFilterValues>({
    enteId: profile?.role === "ente_hr" ? profile.ente_id : null,
    enteIds: [],
  });
  ...
  useEffect(() => {
    const load = async () => {
      ...
      (statusRes.data ?? []).forEach((s: any) => {
        sMap[s.stato_organizzazione_id] = s.descrizione;
      });
  ...
  const byStatus = Object.entries(
    data.reduce((acc: Record<string, number>, r) => {
      acc[r.stato_label] = (acc[r.stato_label] || 0) + 1;
      return acc;
    }, {}),
      ...
      {loading ? (
        ...
        <>
          <div className="grid grid-cols-3 gap-4">
            ...
            <KpiBox
              label="Formalizzate"
              value={data.filter((d) => d.stato_label === "Formalizzata").length}
            />
              ...
              <div className="space-y-2 max-h-[260px] overflow-y-auto">
                {data.map((r, i) => (
                  ...
                  >
                    ...
                    <span
                      className={`text-[10px] font-semibold px-2 py-0.5 rounded ${r.stato_label === "Formalizzata" ? "bg-emerald-500/10 text-emerald-600" : "bg-amber-500/10 text-amber-600"}`}
                    >
                      {r.stato_label}
                    </span>
                  </div>
...
const ProvvedimentiView = () => {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      const enriched = (provRes.data ?? [])
        ...
        .sort(
          (a: any, b: any) =>
            new Date(b.data_adozione_provvedimento).getTime() -
            new Date(a.data_adozione_provvedimento).getTime(),
        );
  ...
  const byMonth = Object.entries(
    data.reduce((acc: Record<string, number>, r) => {
      const m = r.data_adozione_provvedimento
        ? new Date(r.data_adozione_provvedimento).toLocaleDateString("it-IT", {
            month: "short",
            year: "2-digit",
          })
        : "N/D";
  ...
  return loading ? (
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-card border rounded-xl p-5">
          <p className="text-xs font-semibold text-muted-foreground mb-3">
            Provvedimenti per mese di adozione
          </p>
          <ResponsiveContainer width="100%" height={260}>
              ...
              <div key={i} className="flex items-start gap-3 bg-muted/30 rounded-lg px-3 py-2">
                ...
                <div>
                  ...
                  <div className="text-[10px] text-muted-foreground">
                    {r.ente_nome} · Adottato:{" "}
                    {r.data_adozione_provvedimento
                      ? new Date(r.data_adozione_provvedimento).toLocaleDateString("it-IT")
                      : "N/D"}
                  </div>
                </div>
...
const FteView = () => (
  <EmptyDataView
    ...
    description="I dati sui FTE programmati e assegnati per profilo di ruolo non sono ancora stati caricati nel sistema. Questa sezione sarà attiva non appena gli enti inseriranno i dati relativi all'allocazione delle risorse."
  />
);
...
const CoperturaView = () => {
  ...
  useEffect(() => {
    sipoFrom("lk_sipo_copertura_profili_di_ruolo")
      .select("*")
      .then(({ data: d }) => {
        setData(d ?? []);
        setLoading(false);
  ...
  return loading ? (
    ...
    <div className="space-y-4">
      ...
      <div className="bg-card border rounded-xl p-5">
        <p className="text-xs font-semibold text-muted-foreground mb-3">
          Range di copertura profili di ruolo
        </p>
...
const FabbisognoView = () => (
  <EmptyDataView
    title="Fabbisogno per Profilo"
    description="L'analisi del fabbisogno richiede i dati FTE programmati e assegnati per profilo di ruolo. Questa sezione sarà popolata non appena gli enti completeranno la mappatura delle risorse nel sistema SIPrO."
  />
);
...
const CatalogoView = () => {
  ...
  const byComparto = Object.entries(
    data.reduce((acc: Record<string, number>, r) => {
      acc[r.comparto ?? "N/D"] = (acc[r.comparto ?? "N/D"] || 0) + 1;
      return acc;
    }, {}),
  ).map(([name, value]) => ({ name, value }));
      ...
      <div className="bg-card border rounded-xl p-5">
        ...
        <div className="space-y-1.5 max-h-[400px] overflow-y-auto">
          {data.map((r, i) => (
            ...
            >
              <div className="text-[11px] font-medium text-foreground">
                {r[labelField] ?? r.descrizione ?? "N/D"}
              </div>
              {r.codice && (
...
const EvoluzioneView = () => {
  ...
  useEffect(() => {
    sipoFrom("lk_sipo_profili_di_ruolo")
      .select("*")
      .then(({ data: d }) => {
        setData(d ?? []);
        setLoading(false);
      ...
      <div className="bg-card border rounded-xl p-5">
        ...
        <ResponsiveContainer width="100%" height={260}>
          <PieChart>
            <Pie
              data={[
                { name: "Attivi", value: active },
                { name: "Eliminati", value: eliminated },
              ]}
              dataKey="value"
              nameKey="name"
              cx="50%"
...
const minervaMap: Record<string, { tableName: string; title: string; labelField: string }> = {
  "sipro-famiglie": {
    tableName: "lk_minerva_famiglia_professionale",
    title: "Famiglie Professionali",
    labelField: "descrizione",
  },
  "sipro-profili-minerva": {
    tableName: "lk_minerva_profilo_professionale",
    title: "Profili Professionali Minerva",
    labelField: "descrizione",
  },
  "sipro-ambiti-ruolo": {
    tableName: "lk_minerva_ambito_ruolo",
    title: "Ambiti di Ruolo",
    labelField: "descrizione",
  },
  "sipro-aree-contrattuali": {
    tableName: "lk_minerva_area_contrattuale",
    ...
    <div className="flex items-center justify-center p-8">
      <div className="bg-card border rounded-lg p-10 text-center max-w-md">
        ...
        <p className="text-sm text-muted-foreground">
          L'indicatore "{indicatorId}" non dispone ancora di una visualizzazione dedicata.
        </p>
      </div>
    </div>
  );
