...
export const MinervaGapAnalysisSection = () => {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      const compMap = Object.fromEntries(
        (competenze ?? []).map((c) => [c.codice, c.titolo ?? c.codice]),
      );
      const profMap = Object.fromEntries(
        (profili ?? []).map((p) => [p.codice, p.nome ?? p.codice]),
      );
      ...
      const mapped = Object.values(byProfilo)
        .map((p) => ({
  ...
  const avgGap = gapData.length
    ? +(gapData.reduce((s, d) => s + d.gapMedio, 0) / gapData.length).toFixed(2)
    : 0;
  ...
  return (
    <div className="p-4 space-y-4">
      <div className="grid grid-cols-3 gap-3">
        {[
          {
            label: "Gap Medio Competenze",
            value: avgGap,
            color: avgGap > 1 ? "text-red-600" : "text-green-600",
          },
          { label: "Competenze Critiche (gap>1)", value: totalCritical, color: "text-amber-600" },
          {
            label: "Profili con Gap Critico",
            value: gapData.filter((d) => d.critiche > 0).length,
          ...
          <PaginatedTable
            ...
            columns={[
              ...
              { key: "gapMedio", header: "Gap Medio", align: "right" },
              {
                ...
                align: "right",
                render: (r: any) => (
                  <span className={r.gapMax > 1.5 ? "text-red-600 font-semibold" : ""}>
                    {r.gapMax}
                  </span>
                ),
              },
