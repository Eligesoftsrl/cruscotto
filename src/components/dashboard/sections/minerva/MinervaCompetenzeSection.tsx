  ...
  useEffect(() => {
    const load = async () => {
      ...
      bridge.forEach((b: any) => {
        if (b.livello_target) {
          gradCount[b.livello_target] = (gradCount[b.livello_target] || 0) + 1;
        }
      });
      setGradingDist(
        Object.entries(gradCount)
          .map(([level, count]) => ({ level: `Livello ${level}`, count }))
      ...
      setCoverageRate(
        competenze.length > 0 ? Math.round((mappedComps.size / competenze.length) * 100) : 0,
      );
    ...
    <div className="p-4 space-y-4">
      ...
      <div className="grid grid-cols-4 gap-3">
          ...
          <div key={t.name} className="tableau-card">
            <div className="p-4 text-center">
              <div
                ...
                style={{ color: TYPE_COLORS[t.name] ?? "hsl(var(--foreground))" }}
              >
                {t.value}
              </div>
        ...
        <div className="tableau-card">
          <div className="p-4 text-center">
            <div
              className={`text-2xl font-bold ${coverageRate >= 80 ? "text-green-600" : coverageRate >= 50 ? "text-amber-600" : "text-red-600"}`}
            >
              {coverageRate}%
            </div>
          ...
          <div className="p-4" style={{ height: 280 }}>
            <ResponsiveContainer>
              <PieChart>
                ...
                >
                  {compByType.map((e, i) => (
                    <Cell key={i} fill={TYPE_COLORS[e.name] ?? "hsl(200,50%,50%)"} />
                  ))}
                </Pie>
                <Tooltip />
                <Legend />
            ...
            columns={[
              ...
              { key: "area", header: "Area" },
              {
                ...
                render: (r: any) => (
                  <Badge
                    variant="outline"
                    className="text-[10px]"
                    style={{ borderColor: TYPE_COLORS[r.tipo], color: TYPE_COLORS[r.tipo] }}
                  >
                    {r.tipo}
                  </Badge>
                ),
              },
              {
                ...
                render: (r: any) => (
                  <span
                    className={
                      r.mappata === "Sì" ? "text-green-600 font-semibold" : "text-muted-foreground"
                    }
                  >
                    {r.mappata}
                  </span>
