...
export const MinervaAssessmentSection = () => {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      const enteMap = Object.fromEntries(
        (enti ?? []).map((e: any) => [e.id_ente, e.denominazione]),
      );

      const completati = assess.filter((a: any) => a.stato === "completato");
      const inCorso = assess.filter((a: any) => a.stato === "in_corso");
      const gapValues = completati
        .filter((a: any) => a.gap_medio != null)
        .map((a: any) => Number(a.gap_medio));
      const avgGap = gapValues.length
        ? +(gapValues.reduce((s: number, v: number) => s + v, 0) / gapValues.length).toFixed(2)
        : 0;
      const totValutati = assess.reduce(
        (s: number, a: any) => s + (a.nr_dipendenti_valutati ?? 0),
        0,
      );

      setKpis({
        totCicli: assess.length,
        completati: completati.length,
        inCorso: inCorso.length,
        gapMedio: avgGap,
        dipValutati: totValutati,
      });
      ...
      completati.forEach((a: any) => {
        if (!byEnte[a.id_ente])
          byEnte[a.id_ente] = { gaps: [], nome: enteMap[a.id_ente] ?? `Ente ${a.id_ente}` };
        if (a.gap_medio != null) byEnte[a.id_ente].gaps.push(Number(a.gap_medio));
      });
      setGapByEnte(
        Object.values(byEnte)
          .map((e) => ({
      ...
      setTrendData(
        Object.entries(byAnno)
          .map(([anno, v]) => ({
            anno,
            cicli: v.count,
            gapMedio: v.gaps.length
              ? +(v.gaps.reduce((s, g) => s + g, 0) / v.gaps.length).toFixed(2)
              : 0,
          }))
      ...
      <div className="grid grid-cols-5 gap-3">
        {[
          ...
          { label: "In Corso", value: kpis.inCorso, color: "text-amber-600" },
          {
            label: "Gap Medio",
            value: kpis.gapMedio,
            color: kpis.gapMedio > 1 ? "text-red-600" : "text-green-600",
          },
            ...
            <ResponsiveContainer>
              <LineChart data={trendData}>
                ...
                <ReferenceLine
                  ...
                  stroke="hsl(0,70%,55%)"
                  strokeDasharray="5 5"
                  label={{ value: "Soglia critica", fontSize: 9, fill: "hsl(0,70%,55%)" }}
                />
                <Line
                  type="monotone"
                  dataKey="gapMedio"
          ...
          <PaginatedTable
            ...
            columns={[
              ...
              { key: "ciclo", header: "Ciclo" },
              {
                ...
                render: (r: any) => (
                  <Badge
                    variant={r.stato === "completato" ? "default" : "secondary"}
                    className="text-[10px]"
                  >
              ...
              { key: "valutati", header: "Valutati", align: "right" as const },
              {
                ...
                render: (r: any) => (
                  <span
                    className={
                      r.gap !== "—" && parseFloat(r.gap) > 1 ? "text-red-600 font-semibold" : ""
                    }
                  >
                    {r.gap}
                  </span>
