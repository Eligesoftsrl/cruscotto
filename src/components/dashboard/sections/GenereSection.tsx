...
export const GenereSection = () => {
  ...
  const dirUomini = generePerQualifica.filter(r => r.qualifica.includes("Dirig")).reduce((s, r) => s + r.uomini, 0);
  const dirDonne = generePerQualifica.filter(r => r.qualifica.includes("Dirig")).reduce((s, r) => s + r.donne, 0);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 sm:grid-cols-4">
        <KpiCard title="% Donne Totale" value={kpiOverview.donnePerc} suffix="%" icon={UserCheck} accent />
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        <div className="chart-container">
          ...
          <ResponsiveContainer width="100%" height={250}>
            <PieChart>
              <Pie data={genderTotal} cx="50%" cy="50%" innerRadius={55} outerRadius={90} paddingAngle={4} dataKey="value">
                {genderTotal.map((_, i) => <Cell key={i} fill={GENDER_COLORS[i]} />)}
              </Pie>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b">
              <th className="px-4 py-2.5 text-left text-xs font-semibold text-muted-foreground">Qualifica</th>
          ...
          <tbody>
            {generePerQualifica.map(r => {
              ...
              return (
                <tr key={r.qualifica} className="border-b last:border-0 hover:bg-muted/50 transition-colors">
                  ...
                  <td className="px-4 py-2.5 text-right font-semibold">{tot.toLocaleString("it-IT")}</td>
