  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-12 gap-3">
        {[
          { label: "Cessazioni previste 2024-2030", value: totPreviste.toLocaleString("it-IT"), icon: CalendarClock, color: "hsl(var(--chart-orange))" },
          { label: "Anno di picco", value: piccoAnno.anno, sub: `${piccoAnno.cessazioni} cessazioni`, icon: AlertTriangle, color: "hsl(var(--chart-red))" },
          { label: "% organico attuale", value: `${percPersonale}%`, icon: Users, color: "hsl(var(--chart-blue))" },
          { label: "Media annua prevista", value: Math.round(totPreviste / 7).toLocaleString("it-IT"), icon: TrendingDown, color: "hsl(var(--chart-teal))" },
        ].map((k, i) => (
      ...
      <div className="grid grid-cols-12 gap-3">
        <div className="col-span-6 bg-card border rounded-lg p-4">
          <h3 className="text-xs font-semibold text-foreground mb-3">Previsione cessazioni annue e cumulate</h3>
          <ResponsiveContainer width="100%" height={280}>
            <BarChart data={previsioneCessazioni}>
              ...
              <Bar yAxisId="left" dataKey="cessazioni" name="Cessazioni annue" fill="hsl(var(--chart-red))" radius={[3, 3, 0, 0]} fillOpacity={0.8} />
              <Line yAxisId="right" type="monotone" dataKey="cumulate" name="Cumulate" stroke="hsl(var(--chart-navy))" strokeWidth={2} dot={{ r: 3 }} />
            </BarChart>
          </ResponsiveContainer>
        </div>

        <div className="col-span-6 bg-card border rounded-lg p-4">
          <h3 className="text-xs font-semibold text-foreground mb-3">Cessazioni previste per fascia d'età</h3>
          <ResponsiveContainer width="100%" height={280}>
      ...
      <div className="bg-card border rounded-lg p-4">
        <h3 className="text-xs font-semibold text-foreground mb-3">Analisi per scenari (2024–2030)</h3>
        <ResponsiveContainer width="100%" height={280}>
          <AreaChart data={scenari}>
            ...
            <Tooltip contentStyle={{ background: "hsl(var(--card))", border: "1px solid hsl(var(--border))", borderRadius: 8, fontSize: 11 }} />
            <Legend iconType="circle" iconSize={7} wrapperStyle={{ fontSize: 11 }} />
            <Area type="monotone" dataKey="pessimistico" name="Pessimistico" fill="hsl(var(--chart-red))" stroke="hsl(var(--chart-red))" fillOpacity={0.1} strokeDasharray="4 4" />
            <Area type="monotone" dataKey="base" name="Base" fill="hsl(var(--chart-blue))" stroke="hsl(var(--chart-blue))" fillOpacity={0.15} strokeWidth={2} />
            <Area type="monotone" dataKey="ottimistico" name="Ottimistico" fill="hsl(var(--chart-teal))" stroke="hsl(var(--chart-teal))" fillOpacity={0.1} strokeDasharray="4 4" />
          </AreaChart>
        </ResponsiveContainer>
      </div>
    </div>
