...
export const InpaBandiSection = () => {
  ...
  bandi.forEach((b) => {
    byTipo[b.tipo_procedura ?? "Altro"] = (byTipo[b.tipo_procedura ?? "Altro"] || 0) + 1;
  });
  ...
  bandi.forEach((b) => {
    byRegione[b.regione ?? "N/D"] =
      (byRegione[b.regione ?? "N/D"] || 0) + (Number(b.num_posti) || 0);
  });
  const barData = Object.entries(byRegione)
    .map(([regione, posti]) => ({ regione, posti }))
  ...
  const trendData = Object.entries(byAnno)
    .sort()
    .map(([anno, count]) => ({ anno, bandi: count }));
