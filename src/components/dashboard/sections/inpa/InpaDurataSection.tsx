...
export const InpaDurataSection = () => {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      const mapped = bandi
        .filter((b: any) => b.data_pubblicazione && b.data_scadenza)
        .map((b: any) => {
      ...
      if (mapped.length)
        setAvg(Math.round(mapped.reduce((s, d) => s + d.durata, 0) / mapped.length));
    };
    load();
  }, [enteIds]);
      ...
      <div className="grid grid-cols-3 gap-3">
        <div className="tableau-card">
          <div className="p-4 text-center">
            ...
            <div className="text-[11px] text-muted-foreground">Durata Media (gg)</div>
          </div>
        </div>
        <div className="tableau-card">
          <div className="p-4 text-center">
            <div className="text-2xl font-bold">
              {data.length ? Math.min(...data.map((d) => d.durata)) : "-"}
            </div>
            <div className="text-[11px] text-muted-foreground">Durata Minima (gg)</div>
          </div>
        </div>
        <div className="tableau-card">
          <div className="p-4 text-center">
            <div className="text-2xl font-bold">
              {data.length ? Math.max(...data.map((d) => d.durata)) : "-"}
            </div>
            <div className="text-[11px] text-muted-foreground">Durata Massima (gg)</div>
          </div>
        </div>
      </div>
      <div className="tableau-card">
          ...
          <ResponsiveContainer>
            <BarChart data={data.slice(0, 20)} layout="vertical" margin={{ left: 140 }}>
              ...
              <XAxis
                type="number"
                tick={{ fontSize: 11 }}
                label={{ value: "Giorni", position: "bottom", fontSize: 11 }}
              />
              ...
              <ReferenceLine
                ...
                stroke="hsl(0,70%,55%)"
                strokeDasharray="5 5"
                label={{ value: `Media: ${avg}gg`, fontSize: 10 }}
              />
              <Bar
                dataKey="durata"
                name="Durata (gg)"
