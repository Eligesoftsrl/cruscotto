...
export const TassoSostituzioneSection = () => {
  ...
  const media = (
    serieConSostituzione.reduce((s, r) => s + r.tassoSostituzione, 0) / serieConSostituzione.length
  ).toFixed(1);
  ...
  return (
    <div className="space-y-4">
      <KpiGrid>
        {[
          {
            label: "Tasso Sostituzione 2023",
            value: `${ultimo.tassoSostituzione}%`,
            icon: RefreshCw,
            color: "hsl(var(--chart-blue))",
            sub: `${parseFloat(delta) >= 0 ? "+" : ""}${delta} pp vs anno prec.`,
          },
          {
            label: "Media periodo",
            value: `${media}%`,
          ...
          {
            label: "Anni con ricambio positivo",
            value: `${anniSopra100} / ${serieConSostituzione.length}`,
            icon: Target,
            color: "hsl(var(--chart-teal))",
          },
          {
            label: "Variazione vs anno prec.",
            value: `${parseFloat(delta) >= 0 ? "+" : ""}${delta} pp`,
            icon: TrendingUp,
            color: parseFloat(delta) >= 0 ? "hsl(var(--chart-teal))" : "hsl(var(--chart-red))",
          },
        ].map((k, i) => (
          <KpiStat
            key={i}
        ...
        <div className="col-span-7 bg-card border rounded-lg p-4">
          ...
          <ResponsiveContainer width="100%" height={300}>
            <ComposedChart data={serieConSostituzione}>
              ...
              />
              <Bar
                dataKey="tassoSostituzione"
                name="Tasso Sostituzione %"
                barSize={28}
                radius={[4, 4, 0, 0]}
      ...
      <div className="bg-card border rounded-lg p-4">
        <h3 className="text-xs font-semibold text-foreground mb-3">
          Dettaglio Tasso di Sostituzione
        </h3>
              ...
              {serieConSostituzione.map((r) => (
                ...
                >
                  ...
                  <td className="px-4 py-2.5 text-right">
                    <span
                      className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-semibold ${r.tassoSostituzione >= 100 ? "bg-emerald-100 text-emerald-700" : "bg-red-100 text-red-700"}`}
                    >
                      {r.tassoSostituzione}%
                    </span>
                  </td>
