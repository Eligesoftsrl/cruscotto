import { serieStoricaTurnover, kpiOverview } from "@/data/mockData";
...
export const TassoSostituzioneSection = () => {
  // Tasso di sostituzione = assunti / cessati * 100
  const serieConSostituzione = serieStoricaTurnover.map((r) => ({
    ...r,
    tassoSostituzione: parseFloat(((r.assunti / r.cessati) * 100).toFixed(1)),
  }));

  const ultimo = serieConSostituzione[serieConSostituzione.length - 1];
  const penultimo = serieConSostituzione[serieConSostituzione.length - 2];
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-12 gap-3">
        {[
          { label: "Tasso Sostituzione 2023", value: `${ultimo.tassoSostituzione}%`, icon: RefreshCw, color: "hsl(var(--chart-blue))", sub: `${parseFloat(delta) >= 0 ? "+" : ""}${delta} pp vs 2022` },
      ...
      {/* Main chart: Tasso sostituzione con reference line a 100% */}
      <div className="grid grid-cols-12 gap-3">
        <div className="col-span-7 bg-card border rounded-lg p-4">
          <h3 className="text-xs font-semibold text-foreground mb-3">Tasso di Sostituzione (Assunti/Cessati × 100) — 2016–2023</h3>
          <ResponsiveContainer width="100%" height={300}>
            <ComposedChart data={serieConSostituzione}>
              ...
              <Bar dataKey="tassoSostituzione" name="Tasso Sostituzione %" barSize={28} radius={[4, 4, 0, 0]}>
                {serieConSostituzione.map((entry, i) => (
                  <rect key={i} fill={entry.tassoSostituzione >= 100 ? "hsl(var(--chart-teal))" : "hsl(var(--chart-red))"} />
                ))}
              </Bar>
            </ComposedChart>
          </ResponsiveContainer>
        ...
        {/* Confronto Assunti vs Cessati area */}
