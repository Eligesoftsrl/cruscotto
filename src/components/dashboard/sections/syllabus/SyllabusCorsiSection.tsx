...
export const SyllabusCorsiSection = () => {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      setByCategoria(Object.entries(cat).map(([name, value]) => ({ name: name.substring(0, 22), value })).sort((a, b) => b.value - a.value));
      setByLivello(Object.entries(liv).map(([name, value]) => ({ name, value })).sort((a, b) => b.value - a.value));

      setCorsi(data.map((c: any) => ({
  ...
  return (
    <div className="p-4 space-y-4">
      <div className="grid grid-cols-2 gap-3">
        ...
        <div className="tableau-card"><div className="p-4 text-center"><div className="text-2xl font-bold">{totals.competenze}</div><div className="text-[11px] text-muted-foreground">Competenze Coperte</div></div></div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="tableau-card">
          ...
          <div className="p-4" style={{ height: 280 }}>
            <ResponsiveContainer>
              <PieChart>
                <Pie data={byCategoria} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={90} label={({ name, percent }) => `${name.substring(0, 14)} ${(percent * 100).toFixed(0)}%`} labelLine={false} fontSize={9}>
