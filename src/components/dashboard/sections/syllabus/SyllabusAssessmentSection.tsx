  ...
  useEffect(() => {
    const load = async () => {
      const partecipazioni = await fetchSyllabusPartecipazioni(
        "id, esito_finale, id_competenza, livello_da, livello_a",
      );
      ...
      partecipazioni.forEach((p: any) => {
        ...
        if ((p.livello_a ?? 0) > (p.livello_da ?? 0)) {
          comp[c].miglioramento++;
          miglioramenti++;
        }
      });

      setData(
        Object.entries(byEsito)
          .map(([esito, count]) => ({ esito, count }))
      ...
      const compl = partecipazioni.filter(
        (p: any) => p.esito_finale === "Superato" || p.esito_finale === "Completato",
      ).length;
      setTotals({
        partecipanti: tot,
        completati: compl,
        tasso: tot > 0 ? ((compl / tot) * 100).toFixed(1) : "0",
        miglioramenti,
      });
    };
    load();
      ...
      <div className="grid grid-cols-4 gap-3">
        <div className="tableau-card">
          <div className="p-4 text-center">
            <div className="text-2xl font-bold">{totals.partecipanti.toLocaleString()}</div>
            <div className="text-[11px] text-muted-foreground">Partecipazioni</div>
          </div>
        </div>
        <div className="tableau-card">
          <div className="p-4 text-center">
            <div className="text-2xl font-bold">{totals.completati.toLocaleString()}</div>
            <div className="text-[11px] text-muted-foreground">Completamenti</div>
          </div>
        </div>
        <div className="tableau-card">
          <div className="p-4 text-center">
            <div className="text-2xl font-bold">{totals.tasso}%</div>
            <div className="text-[11px] text-muted-foreground">Tasso Completamento</div>
          </div>
        </div>
        <div className="tableau-card">
          <div className="p-4 text-center">
            <div className="text-2xl font-bold">{totals.miglioramenti.toLocaleString()}</div>
            <div className="text-[11px] text-muted-foreground">Miglioramenti Livello</div>
          </div>
        </div>
      </div>
