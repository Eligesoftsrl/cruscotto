...
export const SiproBenchmarkView = () => {
  return (
    <div className="p-4 flex-1 space-y-5">
      ...
      <div className="tableau-card">
        ...
        <div className="p-4">
          <p className="text-[12px] text-muted-foreground leading-relaxed">
            Indicatori normalizzati, percentuali e indici di copertura per il confronto tra cluster di amministrazioni.
            I valori assoluti sono riformulati in chiave comparativa per supportare benchmarking, individuazione criticità sistemiche e diffusione best practice.
          </p>
        </div>
      </div>
            ...
            <div className="h-[280px]">
              <ResponsiveContainer width="100%" height="100%">
                <RadarChart data={radarBenchmarkData} cx="50%" cy="50%" outerRadius="65%">
                  ...
                  <Radar name="Padova" dataKey="padova" stroke="hsl(var(--chart-blue))" fill="hsl(var(--chart-blue))" fillOpacity={0.1} strokeWidth={2} />
                  <Radar name="Brescia" dataKey="brescia" stroke="hsl(var(--chart-teal))" fill="hsl(var(--chart-teal))" fillOpacity={0.1} strokeWidth={2} />
                  <Radar name="Media Cluster" dataKey="media" stroke="hsl(var(--chart-orange))" fill="hsl(var(--chart-orange))" fillOpacity={0.05} strokeWidth={1.5} strokeDasharray="4 4" />
            ...
            <div className="h-[280px]">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={clusterCoverageData} barCategoryGap="20%">
                  ...
                  <XAxis dataKey="ente" tick={{ fontSize: 10, fill: "hsl(var(--muted-foreground))" }} />
                  <YAxis domain={[0, 100]} tick={{ fontSize: 10, fill: "hsl(var(--muted-foreground))" }} tickFormatter={(v: number) => `${v}%`} />
                  <Tooltip contentStyle={{ fontSize: 10, background: "hsl(var(--card))", border: "1px solid hsl(var(--border))" }} formatter={(v: number) => `${v}%`} />
                  <Legend iconType="square" iconSize={10} wrapperStyle={{ fontSize: 10 }} />
                  <Bar dataKey="copertura" name="Copertura Ente" fill="hsl(var(--chart-blue))" radius={[3, 3, 0, 0]} />
                  <Bar dataKey="cluster" name="Media Cluster" fill="hsl(var(--chart-orange) / 0.5)" radius={[3, 3, 0, 0]} />
                </BarChart>
              </ResponsiveContainer>
            </div>
          </div>
      ...
      {/* Digital maturity */}
      <div className="tableau-card">
        <div className="tableau-card-header">Indice Medio di Maturità Digitale (IMDP) per Ente vs Cluster</div>
        <div className="tableau-card-body">
          <div className="h-[220px]">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={digitalMaturityData} barCategoryGap="25%">
                ...
                <Tooltip contentStyle={{ fontSize: 10, background: "hsl(var(--card))", border: "1px solid hsl(var(--border))" }} />
                <Legend iconType="square" iconSize={10} wrapperStyle={{ fontSize: 10 }} />
                <Bar dataKey="indice" name="IMDP Ente" fill="hsl(var(--chart-teal))" radius={[3, 3, 0, 0]} />
                <Bar dataKey="cluster_media" name="Media Cluster" fill="hsl(var(--chart-orange) / 0.4)" radius={[3, 3, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </div>
          ...
          <table className="w-full text-[11px]">
            <thead>
              <tr className="border-b border-border bg-muted/30">
                ...
                <th className="text-left px-3 py-2 font-bold text-muted-foreground">Macro-Criticità</th>
                <th className="text-left px-3 py-2 font-bold text-muted-foreground">Incidenza %</th>
                <th className="text-left px-3 py-2 font-bold text-muted-foreground">Enti Coinvolti</th>
                ...
                <tr key={c.criticita} className={i % 2 === 0 ? "" : "bg-muted/15"}>
                  ...
                  <td className="px-3 py-2">
                    <div className="h-2 w-full bg-muted rounded-full overflow-hidden">
                      <div className="h-full rounded-full" style={{ width: `${c.incidenza}%`, background: c.incidenza > 30 ? "hsl(var(--chart-orange))" : "hsl(var(--chart-blue))" }} />
                    </div>
                  </td>
                </tr>
              ))}
