...
export const AnalisiGenereSection = () => {
  ...
  const gapData = generePerQualifica.map((r) => ({
    ...
    percDonne:
      r.uomini + r.donne > 0 ? parseFloat(((r.donne / (r.uomini + r.donne)) * 100).toFixed(1)) : 0,
    gap:
      r.uomini + r.donne > 0
        ? parseFloat((((r.donne - r.uomini) / (r.uomini + r.donne)) * 100).toFixed(1))
        : 0,
  }));
    ...
    <div className="space-y-4">
      ...
      <KpiGrid>
        {[
          ...
          {
            label: "% Uomini",
            value: `${(100 - parseFloat(percDonne)).toFixed(1)}%`,
            icon: Users,
            color: "hsl(var(--chart-blue))",
          },
          {
            label: "Gender gap index",
            value: `${parseFloat(genderGap) >= 0 ? "+" : ""}${genderGap}%`,
            icon: Scale,
            color: "hsl(var(--chart-purple))",
          },
          {
            label: "Qualifica più bilanciata",
            value: gapData.reduce((a, b) => (Math.abs(b.gap) < Math.abs(a.gap) ? b : a)).qualifica,
            icon: BarChart3,
      ...
      <div className="grid grid-cols-12 gap-3">
        ...
        <div className="col-span-7 bg-card border rounded-lg p-4">
          <h3 className="text-xs font-semibold text-foreground mb-3">
            Distribuzione Genere per Qualifica
          </h3>
          ...
          <div className="space-y-3 mt-4">
            {gapData.map((r) => (
              <div key={r.qualifica}>
                <div className="flex justify-between text-[11px] mb-1">
                  ...
                  <span
                    className={`font-semibold ${r.gap >= 0 ? "text-emerald-600" : "text-red-500"}`}
                  >
                    {r.gap >= 0 ? "+" : ""}
                    {r.gap}%
                  </span>
                ...
                <div className="h-4 bg-muted rounded-full overflow-hidden relative">
                  ...
                  {r.gap >= 0 ? (
                    <div
                      ...
                      style={{
                        left: "50%",
                        width: `${Math.min(Math.abs(r.gap), 50)}%`,
                        background: "hsl(var(--chart-red))",
                      }}
                    />
                  ) : (
                    <div
            ...
            <div className="flex items-center justify-center gap-4 text-[10px] text-muted-foreground mt-2">
              <div className="flex items-center gap-1">
                <div className="w-3 h-2 rounded" style={{ background: "hsl(var(--chart-blue))" }} />{" "}
                Più uomini
              </div>
              <div className="flex items-center gap-1">
                <div className="w-3 h-2 rounded" style={{ background: "hsl(var(--chart-red))" }} />{" "}
                Più donne
              </div>
            </div>
          </div>
        ...
        <ResponsiveContainer width="100%" height={260}>
          <BarChart data={divergingEta} layout="vertical" margin={{ left: 10 }}>
            ...
            <XAxis
              type="number"
              tick={{ fontSize: 10, fill: "hsl(var(--muted-foreground))" }}
              tickFormatter={(v) => Math.abs(v).toLocaleString("it-IT")}
            />
            <YAxis
              type="category"
              dataKey="fascia"
            ...
            <Tooltip
              ...
              formatter={(val: number) => [Math.abs(val).toLocaleString("it-IT"), undefined]}
            />
            <Legend iconType="square" iconSize={8} wrapperStyle={{ fontSize: 11 }} />
              ...
              {gapData.map((r) => (
                ...
                >
                  ...
                  <td className="px-4 py-2.5 text-right">
                    <span
                      className={`inline-flex px-2 py-0.5 rounded-full text-xs font-semibold ${r.gap >= 0 ? "bg-emerald-100 text-emerald-700" : "bg-red-100 text-red-700"}`}
                    >
                      {r.gap >= 0 ? "+" : ""}
                      {r.gap}%
                    </span>
