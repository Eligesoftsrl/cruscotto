...
export const KpiAbilitantiSection = () => {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      const result: DimBlock[] = Object.entries(byDim)
        .map(([dim, kpis]) => {
          const meta = DIM_META[dim] ?? { label: dim, color: "hsl(210,15%,60%)" };
          const nonStruct = kpis.filter((k) => k.categoria !== "strutturale");
          const avgRate = nonStruct.length
            ? Math.round(nonStruct.reduce((s, k) => s + k.valore, 0) / nonStruct.length)
            : 0;
      ...
      if (latestRows.length > 0) {
        ...
        const avgIndices = allIndices[0].map((idx, i) => ({
          ...
          valore:
            Math.round(
              (allIndices.reduce((s, idxs) => s + idxs[i].valore, 0) / allIndices.length) * 10,
            ) / 10,
        }));
    ...
    <div className="p-4 space-y-4">
      ...
      <div className="tableau-card">
        <div className="tableau-card-header">
          KPI Abilitanti e di Successo – 41 KPI per Dimensione (Metodologia Ufficiale)
        </div>
        <div className="p-4">
              ...
              <div key={block.dim} className="rounded-lg border border-border overflow-hidden">
                ...
                >
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[10px] text-muted-foreground">
                      ({block.kpis.length} KPI)
                    </span>
                  </div>
                  <div className="flex items-center gap-2">
                    <div className="text-[11px] font-semibold" style={{ color: block.color }}>
                      SR: {block.avgRate}%
                    </div>
                    <div className="w-16 h-2 rounded-full bg-muted overflow-hidden">
                      <div
                        ...
                        style={{
                          width: `${Math.min(100, block.avgRate)}%`,
                ...
                <div className="divide-y divide-border/50 max-h-[280px] overflow-y-auto">
                  {block.kpis.map((kpi) => (
                    <div
                      key={kpi.codice}
                      className="px-4 py-2 flex items-center gap-3 text-[11px] hover:bg-muted/20 transition-colors"
                    >
