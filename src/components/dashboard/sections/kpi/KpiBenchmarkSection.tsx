    ...
    const load = async () => {
      ...
      Object.values(byEnte).forEach((row: any) => {
        ...
        const ente =
          row.denominazione
            ?.replace("Comune di ", "")
            .replace("Università degli Studi di ", "Uni. ")
            ?.substring(0, 18) ?? "N/D";
        ...
        const srAll = srTot.length
          ? Math.round(srTot.reduce((a, b) => a + b, 0) / srTot.length)
          : 0;
        const srAbl = ablVals.length
          ? Math.round(ablVals.reduce((a, b) => a + b, 0) / ablVals.length)
          : 0;
      ...
      kpis.forEach((row: any) => {
        ...
        if (vals.length)
          byClass[cls].push(Math.round(vals.reduce((a, b) => a + b, 0) / vals.length));
      });
      setBenchmarkByClass(
        Object.entries(byClass).map(([cls, vals]) => ({
          classe: cls,
    ...
    <div className="p-4 space-y-4">
      ...
      <div className="tableau-card">
        <div className="tableau-card-header">
          Matrice di Posizionamento: SR Complessivo vs SR Abilitanti
        </div>
          ...
          <ResponsiveContainer>
            <ScatterChart margin={{ top: 20, right: 20, bottom: 40, left: 20 }}>
              ...
              <XAxis
                ...
                tick={{ fontSize: 10 }}
                label={{
                  value: "Success Rate Complessivo (%)",
                  position: "bottom",
                  offset: 20,
                  fontSize: 11,
                }}
                  ...
                  return (
                    <div className="bg-background border border-border rounded p-2 text-[11px] shadow-lg">
                      ...
                      <div
                        ...
                        style={{ color: quadrantColor(d.x, d.y) }}
                      >
                        {d.x >= 50 && d.y >= 50
                          ? "🟢 Maturità"
                          : d.x >= 50
