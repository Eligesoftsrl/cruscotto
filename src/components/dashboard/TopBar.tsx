...
export const TopBar = ({ nav, onNavigate }: TopBarProps) => {
  ...
  const isPannelloOrRapporto = location.pathname.startsWith("/bussola") || location.pathname.startsWith("/rapporto");
    ...
    <div className="sticky top-0 z-40">
      ...
      <header className="bg-[#004080] text-white h-10 flex items-center px-6 text-xs">
        <div className="flex items-center gap-2">
          <div className="w-6 h-6 rounded-full bg-white/20 flex items-center justify-center text-[8px] font-bold">🏛</div>
        ...
        {profile && (
          <div className="flex items-center gap-2">
            ...
            <span className="text-white/80 text-[11px] font-medium truncate max-w-[100px]">{profile.full_name}</span>
            <button onClick={signOut} className="p-1 rounded hover:bg-white/10 transition" title="Esci">
      ...
      <div className="bg-card border-b flex items-center h-12 px-6">
        <div className="flex items-center gap-3 min-w-0">
          <div>
            ...
            <div className="text-[9px] text-muted-foreground leading-tight">Sistema di Monitoraggio HR della Pubblica Amministrazione</div>
          </div>
        </div>
        ...
        <div className="hidden md:flex items-center">
          <img src={headerLogos} alt="Loghi istituzionali: NextGenerationEU, Dipartimento della Funzione Pubblica, PNRR, FORMEZ" className="h-8 object-contain" />
        </div>
        ...
        {profile?.role === "dfp" && (
          <span className="ml-2 px-2.5 py-1 rounded text-[10px] font-bold bg-primary/10 text-primary uppercase tracking-wide">DFP</span>
        )}
      </div>
    </div>
  );
