...
function assessIAC(value: number): { level: string; color: string; text: string } {
  if (value >= 0.75) return {
    level: "Eccellente", color: "hsl(var(--chart-green))",
  ...
  if (value >= 0.55) return {
    level: "Buono", color: "hsl(var(--chart-green))",
  ...
  if (value >= 0.35) return {
    level: "Moderato", color: "hsl(var(--chart-orange))",
  ...
  return {
    level: "Basso", color: "hsl(var(--destructive))",
...
export function useIACCalculation() {
  return useQuery<IACResult>({
    ...
    queryFn: async () => {
      ...
      rows.forEach(r => { if (r.id_ente != null) byEnte.set(r.id_ente, r); });
      ...
      const conProfiliAttivati = unique.filter(e =>
        e.q1_1_adozione_modello && e.q1_1_adozione_modello !== "No" && toNum(e.q1_5_n_profili_definiti) > 0
      ).length;
