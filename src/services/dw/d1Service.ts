...
/* Service D1: calcolo dinamico degli indicatori IAC, IIMP/R, ICPR, ICVC, IACU
   a partire dalle tabelle dw_*. Estratto da useD1Calculations per separare
   l'accesso ai dati dalla logica di presentazione (hook thin). */
...
export async function fetchD1Indicators(filters?: D1Filters): Promise<D1Result> {
  ...
  let kpiQuery = supabase
    ...
    .select(
      "id_ente, denominazione, q1_1_adozione_modello, q1_5_n_profili_definiti, q1_6_n_profili_competenze, q2_5_assessment, q6_tep_personale",
    );
  ...
  let bridgeQuery = supabase
    ...
    .select(
      "id_ente, cod_profilo_di_ruolo, cod_competenza, livello_target, livello_valutato_medio, dipendenti_valutati, dipendenti_totali_profilo",
    );
  ...
  const accreditate = kpiUnique.filter(
    (k) => k.q1_1_adozione_modello && k.q1_1_adozione_modello !== "No",
  ).length;
  const conProfili = kpiUnique.filter(
    (k) =>
      k.q1_1_adozione_modello &&
      k.q1_1_adozione_modello !== "No" &&
      toNum(k.q1_5_n_profili_definiti) > 0,
  ).length;
    ...
    assessment:
      iacValue >= 0.75
        ? {
            ...
            text: `${conProfili} su ${accreditate} amministrazioni hanno attivato i profili. IAC = ${iacValue.toFixed(2).replace(".", ",")}.`,
          }
        : iacValue >= 0.55
          ? {
              level: "Buono",
          ...
          : iacValue >= 0.35
            ? {
                ...
                text: `Solo ${conProfili} su ${accreditate} amministrazioni hanno attivato i profili. IAC = ${iacValue.toFixed(2).replace(".", ",")}.`,
              }
            : {
                level: "Basso",
                color: "hsl(var(--destructive))",
                text: `Solo ${conProfili} su ${accreditate}. IAC = ${iacValue.toFixed(2).replace(".", ",")}. Azione urgente.`,
              },
  };
  ...
  const IIMP_R: IndicatorResult = {
    ...
    subIndicators: [
      {
        key: "Profili definiti",
        value: iimpNorm,
        color: iimpNorm >= 0.5 ? "hsl(var(--chart-teal))" : "hsl(var(--chart-orange))",
      },
      {
        key: "Profili competenze",
        value: immrNorm,
        color: immrNorm >= 0.5 ? "hsl(var(--chart-teal))" : "hsl(var(--chart-orange))",
      },
    ],
    formulaBreakdown: {
    ...
    assessment:
      iimpCombined >= 0.65
        ? {
            level: "Buono",
            color: "hsl(var(--chart-green))",
            text: `${totProfProf} profili e ${totProfRuolo} con competenze su ${totDip} dipendenti. IIMP/R = ${iimpCombined.toFixed(2).replace(".", ",")}.`,
          }
        : iimpCombined >= 0.4
          ? {
              level: "Moderato",
          ...
          : {
              level: "Basso",
              color: "hsl(var(--destructive))",
              text: `Solo ${totProfProf} profili su ${totDip} dipendenti. IIMP/R = ${iimpCombined.toFixed(2).replace(".", ",")}.`,
            },
  };
  ...
  const totConProfilo = Array.from(profilesByEnte.values()).reduce(
    (s, pm) => s + Array.from(pm.values()).reduce((a, b) => a + b, 0),
    0,
  );
    ...
    assessment:
      icprValue >= 0.75
        ? {
            level: "Eccellente",
            color: "hsl(var(--chart-green))",
            text: `${totConProfilo} su ${totDip} dipendenti con profilo. ICPR = ${icprValue.toFixed(2).replace(".", ",")}.`,
          }
        : icprValue >= 0.5
          ? {
              level: "Buono",
          ...
          : icprValue >= 0.3
            ? {
                level: "Moderato",
                color: "hsl(var(--chart-orange))",
                text: `Solo ${totConProfilo} su ${totDip} dipendenti con profilo. ICPR = ${icprValue.toFixed(2).replace(".", ",")}.`,
              }
            : {
                level: "Basso",
                color: "hsl(var(--destructive))",
                text: `Solo ${totConProfilo} su ${totDip} dipendenti. ICPR = ${icprValue.toFixed(2).replace(".", ",")}. Sistema scarsamente utilizzato.`,
              },
  };
  ...
  const icvcDrilldown: DrilldownData = {
    ...
    rows: Array.from(valutatiByEnte.entries()).map(([eid, pm]) => ({
      ...
      extra: {
        ...
        dip_totali: profilesByEnte.get(eid)
          ? Array.from(profilesByEnte.get(eid)!.values()).reduce((a, b) => a + b, 0)
          : 0,
      },
    })),
  };
    ...
    assessment:
      icvcValue >= 0.6
        ? {
            level: "Buono",
            color: "hsl(var(--chart-green))",
            text: `${totValutati} su ${totDip} dipendenti valutati. ICVC = ${icvcValue.toFixed(2).replace(".", ",")}.`,
          }
        : icvcValue >= 0.35
          ? {
              level: "Moderato",
              color: "hsl(var(--chart-orange))",
              text: `Solo ${totValutati} su ${totDip} dipendenti valutati. ICVC = ${icvcValue.toFixed(2).replace(".", ",")}. Necessario estendere l'assessment.`,
            }
          : {
              level: "Basso",
              color: "hsl(var(--destructive))",
              text: `Solo ${totValutati} su ${totDip} dipendenti valutati. ICVC = ${icvcValue.toFixed(2).replace(".", ",")}. Azione urgente.`,
            },
  };
  ...
  const critici = bridge.filter(
    (c) => (c.livello_target ?? 0) - (c.livello_valutato_medio ?? 0) > 1,
  ).length;
  const rateCritici = bridge.length > 0 ? critici / bridge.length : 0;
  const totCompetenze = bridge.length;

  // Group by profilo for drilldown
  const byProfilo: Record<
    string,
    { count: number; sumGap: number; sumReq: number; critici: number }
  > = {};
  bridge.forEach((c) => {
    ...
    assessment:
      iacuValue >= 0.8
        ? {
            level: "Eccellente",
            color: "hsl(var(--chart-green))",
            text: `Copertura competenze al ${(iacuValue * 100).toFixed(0)}%. Solo ${critici} gap critici su ${totCompetenze}. IACU = ${iacuValue.toFixed(2).replace(".", ",")}.`,
          }
        : iacuValue >= 0.6
          ? {
              level: "Buono",
              color: "hsl(var(--chart-green))",
              text: `Copertura al ${(iacuValue * 100).toFixed(0)}%. ${critici} gap critici su ${totCompetenze}. IACU = ${iacuValue.toFixed(2).replace(".", ",")}.`,
            }
          : iacuValue >= 0.4
            ? {
                level: "Moderato",
                color: "hsl(var(--chart-orange))",
                text: `Copertura al ${(iacuValue * 100).toFixed(0)}%. ${critici} gap critici su ${totCompetenze} competenze. IACU = ${iacuValue.toFixed(2).replace(".", ",")}.`,
              }
            : {
                level: "Basso",
                color: "hsl(var(--destructive))",
                text: `Forte mismatch: copertura solo al ${(iacuValue * 100).toFixed(0)}%. ${critici} gap critici. IACU = ${iacuValue.toFixed(2).replace(".", ",")}.`,
              },
  };
